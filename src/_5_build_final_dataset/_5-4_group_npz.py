...
def consolidate_csv_chunks(folder_path, file_prefix, output_file):
    """
    Consolidate multiple CSV chunk files into a single CSV file.
    
    Args:
        folder_path (str): Path to the folder containing CSV chunk files.
        file_prefix (str): Prefix of the CSV chunk files (e.g., 'patch_metrics_chunk').
        output_file (str): Path to save the consolidated CSV file.
    """
    print(f"\n-> Consolidating CSV chunks with prefix '{file_prefix}' in {folder_path}...")
    
    # Collect all CSV chunk files matching the prefix
    chunk_files = sorted(
        [os.path.join(folder_path, f) for f in os.listdir(folder_path) if f.startswith(file_prefix) and f.endswith(".csv")],
        key=lambda x: int(re.search(rf"{file_prefix}_(\d+)\.csv$", os.path.basename(x)).group(1))
    )

    if not chunk_files:
        print(f"No CSV chunks found with prefix '{file_prefix}'. Skipping.")
        return
    ...
    for chunk_file in tqdm(chunk_files, desc=f"Loading {file_prefix}", unit="chunk"):
        # Read CSV chunk into a dataframe
        df = pd.read_csv(chunk_file)
        dataframes.append(df)

    # Concatenate all dataframes into a single dataframe
    print(f"-> Combining {len(dataframes)} CSV chunks...")
    consolidated_df = pd.concat(dataframes, ignore_index=True)

    # Save the final dataframe as a CSV file with a single header
    print(f"-> Saving consolidated CSV to {output_file}...")
    consolidated_df.to_csv(output_file, index=False)

    # Cleanup
    del dataframes, consolidated_df
    gc.collect()
...
def process_slide_folders(slide_ids, folder_name):
    ...
    for slide_id in slide_ids:
        ...
        patch_ids_file = os.path.join(slide_folder, "patch_ids.npy")
        chunked_patch_ids = sorted(
            [os.path.join(slide_folder, f) for f in os.listdir(slide_folder) if f.startswith("patch_ids_chunk") and f.endswith(".npy")]
        )

        if os.path.exists(patch_ids_file):
            print(f"[INFO] Single 'patch_ids.npy' file detected for slide {slide_id}.")
            consolidate_chunks(slide_folder, "masks_chunk", os.path.join(slide_folder, "masks.npz"))
        elif chunked_patch_ids:
            print(f"[INFO] Chunked 'patch_ids_chunk' files detected for slide {slide_id}.")
            ...
            consolidate_csv_chunks(slide_folder, "patch_metrics_chunk", os.path.join(slide_folder, "patch_metrics.csv"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--folder_name", type=str, default="/Volumes/DD_FGS/MICS/data_HE2CellType/CT_DS/ds_slides_cat/ct_1", help="Parent folder containing slide subfolders.")
